...
def main(args=None):
    ...
    # generate labels
    labels = [sig.get_label(include_id=False, max_name_length=sig_max_len)
    ...
    # prop = font_manager.FontProperties(fname =
    ...
    if args.show_sample_labels:
        cbar_pad += 0.1
    cb = plt.colorbar(
        orientation=cbar_orient, shrink=cbar_scale,
    ...
    q, n = S.shape
    if args.show_sample_labels:
        plt.xticks(np.arange(n), sample_labels, size=sample_font_size,
