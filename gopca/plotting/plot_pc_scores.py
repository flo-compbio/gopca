...
def get_argument_parser():
    ...
    g.add_argument('-n', '--sig-name', required = True,
            metavar = str_mv, type = str_type,
            help = 'The GO term ID or name of the signature.')

    g.add_argument('-c', '--components', type = int, nargs = 2,
            default = [1, 2], metavar = int_mv,
    ...
    g = cli.add_reporting_args(parser)
...
def main(args = None):
    ...
    term_ids = set([sig.term[0] for sig in G.signatures])
    sig = None
    idx = None
    if sig_name in term_ids:
        idx = [i for i, s in enumerate(G.signatures) if s.term[0] == sig_name]
        assert len(idx) == 1
        idx = idx[0]
    else:
        sig_name = sig_name.lower()
        idx = [i for i, s in enumerate(G.signatures)
    ...
    #plt.scatter(y1, y2, c = expr,s=fig_marker_size,alpha=fig_marker_alpha,cmap=cmap,vmin=vmin,vmax=vmax)
